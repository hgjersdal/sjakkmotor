...
(defun time-search (game depth qdepth whitep index)
  (let ((move (if (= (n-moves game) 0) (make-chess-move) (aref (played-moves game) (- (n-moves game) 1))))
    ...
    ;;(setf (start-time game) (get-internal-real-time))
    (timed-search game move scale castlings opponent-castlings index castle-row (search-time game))))
