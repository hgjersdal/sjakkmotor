...
(defun analyze (game whitep depth qdepth)
   (with-slots (move-arrays) game
     (let ((moves (aref move-arrays 0)))
       ...
       (let ((eq-index 0))
	 (loop for move across moves
	    with top = (chess-move-evaluation (aref moves 0))
	    when (> (chess-move-evaluation move) (- top 15)) do
	      (incf eq-index))
	 (move-piece game (aref moves (random eq-index)) (if whitep (white-castle game) (black-castle game)) (if whitep 0 7)))
